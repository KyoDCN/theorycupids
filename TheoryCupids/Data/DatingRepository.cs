...
namespace TheoryCupids.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        {
            return await _context.Photos.Where(user => user.UserId == userId).FirstOrDefaultAsync(p => p.IsMain);
        }
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            ...
            switch (messageParams.MessageContainer)
            {
                case "Inbox":
                    messages = messages
                        .Where(u => u.RecipientId == messageParams.UserId && u.RecipientDeleted == false);
                    break;
                case "Outbox":
                    messages = messages
                        .Where(u => u.SenderId == messageParams.UserId && u.SenderDeleted == false);
                    break;
                default:
                    messages = messages
                        .Where(u => u.RecipientId == messageParams.UserId && u.RecipientDeleted == false && u.IsRead == false);
                    break;
            }
